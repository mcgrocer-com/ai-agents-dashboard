...
-- The pg_trgm extension enables trigram-based indexing for pattern matching queries
...
-- This is the primary search column and most commonly used
...
-- Description is also searched but is typically longer text
...
COMMENT ON INDEX idx_scraped_products_name_trgm IS 'GIN trigram index for fast ILIKE pattern matching on product names';
COMMENT ON INDEX idx_scraped_products_description_trgm IS 'GIN trigram index for fast ILIKE pattern matching on product descriptions';
