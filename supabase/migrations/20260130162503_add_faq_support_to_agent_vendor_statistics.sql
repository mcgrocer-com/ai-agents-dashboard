-- Update get_agent_vendor_statistics to support FAQ agent
-- Adds 'faq' as a valid agent_type that maps to faq_status column
...
BEGIN
  ...
  CASE agent_type
    ...
    WHEN 'faq' THEN
      status_col := 'faq_status';
    ELSE
      RAISE EXCEPTION 'Invalid agent_type: %. Must be category, weight_dimension, seo, copyright, or faq', agent_type;
  END CASE;
...
COMMENT ON FUNCTION get_agent_vendor_statistics IS
  ...
  Supports agent types: category, weight_dimension, seo, copyright, faq';
...
-- SELECT get_agent_vendor_statistics('faq', 'all');
